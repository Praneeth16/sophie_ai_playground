...
# Employer Branding & Talent Marketing use cases data
use_cases_data = [
    {"name": "Recruitment Content Creation Engine", "description": "AI-generated job postings, employer branding content, and talent attraction copy"},
    {"name": "SEO-Optimized Job Posting Generator", "description": "Search engine optimized job descriptions and career page content for maximum candidate visibility"}
]

def render_pillar_page():
    """Render Employer Branding & Talent Marketing pillar page with use cases in card format"""
    st.markdown("# Employer Branding & Talent Marketing")
    st.markdown("**AI-powered tools and solutions for employer branding & talent marketing**")
    st.write("")
...
def render_use_case_page(use_case_name):
    """Render an individual use case page for Employer Branding & Talent Marketing"""
    st.markdown(f"# {use_case_name}")
    
    # Back button to pillar page
    col_back, col_spacer = st.columns([2, 8])
    with col_back:
        if st.button("← Back to Employer Branding & Talent Marketing", use_container_width=True):
    ...
    if selected_tool:
        ...
        st.error(f"Use case '{use_case_name}' not found in Employer Branding & Talent Marketing.")
