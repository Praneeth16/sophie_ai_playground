...
def render_pillar_page():
    """Render Technology Transformation pillar page with use cases in card format"""
    st.markdown("# Technology Transformation")
    st.markdown("**AI-powered tools and solutions for technology transformation**")
    st.write("")
...
def render_use_case_page(use_case_name):
    """Render an individual use case page for Technology Transformation"""
    st.markdown(f"# {use_case_name}")
    
    # Back button to pillar page
    col_back, col_spacer = st.columns([2, 8])
    with col_back:
        if st.button("← Back to Technology Transformation", use_container_width=True):
    ...
    if selected_tool:
        ...
        st.error(f"Use case '{use_case_name}' not found in Technology Transformation.")
