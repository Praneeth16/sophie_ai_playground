...
def initialize_llm() -> bool:
    ...
    try:
        ...
        st.error("OPENAI_API_KEY not found in secrets. Please configure it in Streamlit secrets.", )
        openai_ready = False
        openai_api_key = None
    ...
    if openai_ready and (llm is None or getattr(llm, 'openai_api_key', None) != openai_api_key):
        try:
            ...
            return True
        except Exception as e: st.error(f"LLM Init Error: {e}", ); llm = None; llm_with_search = None; return False
...
def get_node_display_name(node_id: str) -> str:
    if node_id == START_NODE_ID: return "START"
    ...
    def agent_node_function(state: WorkflowState) -> WorkflowState:
        ...
        try:
            ...
            if hasattr(result, 'tool_calls') and result.tool_calls:
                 ...
                 warning_msg = f"   WARNING: LLM response included 'tool_calls' ({call_details}). Graph not handling them."; print(warning_msg); st.session_state.execution_log.append(warning_msg); st.warning(warning_msg)
            ...
            if isinstance(response_content, str):
                ...
                else: st.session_state.execution_log.append(f"   WARNING: No routing key found."); response_content += f" {ROUTING_KEY_MARKER} {DEFAULT_ROUTING_KEY}"; st.session_state.execution_log.append(f"  🔧 Appended default key.")
            else: # Should not happen now, but keep as safeguard
                 st.session_state.execution_log.append(f"   ERROR: Extracted content not str ({type(response_content)}).")
...
def compile_graph() -> bool:
    if not llm: st.error("LLM not initialized.", ); return False
    if not st.session_state.nodes: st.warning("No nodes defined.", ); return False
    ...
    try:
        ...
        if not valid_nodes: st.warning("No valid nodes.", ); return False
        ...
        for node_data in valid_nodes:
            ...
            for rule_idx, rule in enumerate(conditional_targets):
                ...
                if key and target_id:
                    if target_id != END_NODE_ID and target_id not in node_ids: st.error(f"Invalid Target: {node_name} '{key}'->'{get_node_display_name(target_id)}'", ); all_targets_valid=False; node_targets_valid=False; continue
                    if key in seen_keys_for_node: st.warning(f" Duplicate key '{key}' in '{node_name}'.", )
                    path_map[key] = target_id; seen_keys_for_node.add(key); print(f"      - If key '{key}' -> {get_node_display_name(target_id)}")
                elif key or target_id: st.warning(f"Node '{node_name}' incomplete rule #{rule_idx+1}. Ignored.", )
            if DEFAULT_ROUTING_KEY not in path_map:
                 if default_target != END_NODE_ID and default_target not in node_ids: st.error(f"Invalid Default Target: {node_name}->'{get_node_display_name(default_target)}'", ); all_targets_valid=False; node_targets_valid=False
        ...
        if not all_targets_valid: st.error("Compilation failed.", ); return False
        ...
        print("✅ Graph compiled successfully!"); st.toast("Workflow compiled!", ); return True
    except Exception as e: st.error(f"Compile error: {e}", ); print(f"Compile Error: {e}"); import traceback; traceback.print_exc(); st.session_state.compiled_graph = None; return False
...
default_values = { "compiled_graph": None, "execution_log": [], "final_state": None, "selected_node_id": None, "recursion_limit": None }
...
def update_node_data_ui(node_id: str, data: Dict[str, Any]):
    ...
    if updated: st.session_state.compiled_graph = None; st.toast(f"Node '{data.get('name', node_id)}' updated.", ); st.session_state.selected_node_id = node_id
...
def delete_node_ui(node_id_to_delete: str):
    ...
    if isinstance(node_list, list):
        ...
        if len(st.session_state.nodes) < original_len:
            ...
            st.toast(f"Deleted {node_name_deleted}")
            return True
        else: 
    ...
    if isinstance(node_list, list):
        for i, node in enumerate(node_list):
            if isinstance(node, dict) and node.get("id") == node_id:
                ...
                node["routing_rules"]["conditional_targets"].append({"output_key": "", "target_node_id": END_NODE_ID})
                st.session_state.compiled_graph = None
...
st.set_page_config(layout="wide", page_title="Visual AI Automation Builder ")
st.markdown('<div class="hero-title">Visual AI Automation Builder</div>', unsafe_allow_html=True)

with st.sidebar: # Sidebar UI (No Change)
    # Initialize LLM using secrets
    ...
    with st.form("add_node_form"):
        ...
        if add_node_submitted:
            if new_node_name_input:
                if any(isinstance(n, dict) and n.get('name') == new_node_name_input for n in st.session_state.nodes): st.warning(f"Name exists.", )
                else: node_id = f"node_{uuid.uuid4().hex[:6]}"; st.session_state.nodes.append({"id": node_id, "name": new_node_name_input, "type": "llm_call", "prompt": f"Task: {new_node_name_input}\nInput: {{input_text}}\n(Use web search if needed.)", "routing_rules": {"default_target": END_NODE_ID, "conditional_targets": []}}); st.session_state.compiled_graph = None; st.session_state.selected_node_id = node_id; st.toast(f"Added: {new_node_name_input}", ); st.rerun()
            else: st.error("Node Name required.", )
    st.divider(); st.header("Example Workflows"); ex_cols = st.columns(2)
    with ex_cols[0]: 
        if st.button("Summarizer", use_container_width=True, key="load_summarizer_btn"):
            if load_workflow(get_simple_summarizer_workflow): st.rerun()
        if st.button("Sentiment", use_container_width=True, key="load_sentiment_btn"):
            if load_workflow(get_sentiment_workflow): st.rerun()
        if st.button("Deep Research", use_container_width=True, key="load_deep_research_btn"):
            if load_workflow(get_deep_research_workflow): st.rerun()
    with ex_cols[1]: 
        if st.button("Classify", use_container_width=True, key="load_classify_btn"):
            if load_workflow(get_classification_workflow): st.rerun()
        if st.button("Adv. Hedge Fund", use_container_width=True, key="load_adv_hedge_btn"):
            if load_workflow(get_enhanced_hedge_fund_workflow): st.rerun()
    st.divider(); st.header("Workflow Control"); compile_disabled = not llm_ready or not st.session_state.nodes; tooltip_compile = "Requires API Key & nodes." if compile_disabled else "Compile workflow.";
    ...
    if st.button("Reset Workflow", use_container_width=True, help=tooltip_reset, key="reset_workflow_btn"):
...
with top_cols[0]: # Graph Vis (No Change)
    ...
    if not st.session_state.nodes: st.info("Add nodes or load an example.")
        ...
        except Exception as e: st.error(f"Graph Error: {e}", ); print(f"Graph Error: {e}"); import traceback; traceback.print_exc()
with top_cols[1]: # Node Config (No Change)
    st.subheader("Node Configuration"); valid_node_ids = [n.get('id') for n in st.session_state.nodes if isinstance(n, dict)]
    ...
    if selected_node_data:
         ...
         with st.container(border=True):
            ...
            with action_cols[0]: 
                if st.button("Add Rule", key=f"add_rule_btn_{node_id}", use_container_width=True):
                    if add_conditional_rule_ui(node_id): st.rerun()
                
                if conditional_targets: 
                    rule_opts = [(idx, f"Rule {idx+1}: '{rule.get('output_key', '')[:10]}...'") for idx, rule in enumerate(conditional_targets)]
                    rule_opts.insert(0, (-1, "Delete Rule..."))
                    selected_rule_idx_to_del = st.selectbox("Delete Rule", options=rule_opts, format_func=lambda x: x[1], label_visibility="collapsed", index=0, key=f"del_rule_sel_{node_id}")
                    
                    if selected_rule_idx_to_del and selected_rule_idx_to_del[0] != -1: 
                        rule_idx_del = selected_rule_idx_to_del[0]
                        if st.button(f"Confirm Del Rule {rule_idx_del+1}", key=f"del_rule_btn_{node_id}_{rule_idx_del}", use_container_width=True, type="secondary"):
                            if delete_conditional_rule_ui(node_id, rule_idx_del): st.rerun()
            
            with action_cols[1]: 
                current_node_index = next((i for i, n in enumerate(st.session_state.nodes) if isinstance(n, dict) and n.get("id") == node_id), -1)
                
                if st.button("Up", key=f"mv_up_{node_id}", disabled=(current_node_index <= 0), use_container_width=True):
                    if move_node_ui(node_id, -1): st.rerun()
                
                if st.button("Down", key=f"mv_dn_{node_id}", disabled=(current_node_index < 0 or current_node_index >= len(st.session_state.nodes) - 1), use_container_width=True):
                    if move_node_ui(node_id, 1): st.rerun()
                
                if st.button("Delete Node", key=f"del_nd_{node_id}", use_container_width=True, type="secondary", help="Delete this node."):
                    if delete_node_ui(node_id): st.rerun()
...
if st.session_state.compiled_graph and llm_ready: st.success("Workflow compiled.", ); run_tooltip = "Run workflow."; run_disabled = False
elif not llm_ready: st.warning("LLM not ready.", ); run_tooltip = "LLM needs API Key."
elif st.session_state.nodes: st.warning("Compile Workflow first.", ); run_tooltip = "Compile first."
...
if st.button("▶️ Run Workflow", disabled=run_disabled, type="primary", help=run_tooltip):
    ...
    with st.spinner("Executing workflow..."):
        try:
            ...
            st.session_state.final_state = final_state_result; st.session_state.execution_log.append("**Workflow Finished**"); st.toast("Finished!")
        except Exception as e:
            err_msg = f"{e}"
            if "must be followed by tool messages" in str(e): err_msg = f"LLM generated 'tool_calls' which graph cannot handle. Error: {e}."; st.error(f"{err_msg}", )
            elif isinstance(e, RecursionError) or "recursion limit" in str(e).lower(): err_msg = f"Recursion limit ({rec_limit}) reached. Error: {e}"; st.error(f"{err_msg}", )
            else: st.error(f"Exec failed: {err_msg}", );
            st.session_state.execution_log.append(f"**WORKFLOW ERROR:** {err_msg}"); print(f"Exec Error: {e}"); import traceback; traceback.print_exc(); st.toast("Failed!", )
...
with results_cols[1]:
    ...
    if final_message_content:
        ...
        elif "ERROR:" in final_message_content: avatar = ""
