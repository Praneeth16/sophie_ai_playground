...
# Conversational AI use cases data
use_cases_data = [
    {"name": "Voice-Enabled Candidate Interaction", "description": "AI-powered voice interfaces for enhanced candidate screening, support, and engagement"}
]

def render_pillar_page():
    """Render Conversational AI pillar page with use cases in card format"""
    st.markdown("# Conversational AI")
    st.markdown("**AI-powered tools and solutions for conversational ai**")
    st.write("")
...
def render_use_case_page(use_case_name):
    """Render an individual use case page for Conversational AI"""
    st.markdown(f"# {use_case_name}")
    
    # Back button to pillar page
    col_back, col_spacer = st.columns([2, 8])
    with col_back:
        if st.button("← Back to Conversational AI", use_container_width=True):
    ...
    if selected_tool:
        ...
        st.error(f"Use case '{use_case_name}' not found in Conversational AI.")
